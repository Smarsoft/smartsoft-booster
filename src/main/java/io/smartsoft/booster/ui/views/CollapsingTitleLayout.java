...
public class CollapsingTitleLayout extends FrameLayout {
    ...
    public CollapsingTitleLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (a.hasValue(R.styleable.CollapsingTitleLayout_expandedMarginStart)) {
            final int marginStart =
                    a.getDimensionPixelSize(R.styleable.CollapsingTitleLayout_expandedMarginStart,
                            0);
            if (isRtl) {
        ...
        if (a.hasValue(R.styleable.CollapsingTitleLayout_expandedMarginEnd)) {
            final int marginEnd =
                    a.getDimensionPixelSize(R.styleable.CollapsingTitleLayout_expandedMarginEnd, 0);
            if (isRtl) {
        ...
        if (a.hasValue(R.styleable.CollapsingTitleLayout_expandedMarginBottom)) {
            mExpandedMarginBottom =
                    a.getDimensionPixelSize(R.styleable.CollapsingTitleLayout_expandedMarginBottom,
                            0);
        }
        ...
        if (a.hasValue(R.styleable.CollapsingTitleLayout_collapsedTextSize)) {
            mCollapsedTitleTextSize =
                    a.getDimensionPixelSize(R.styleable.CollapsingTitleLayout_collapsedTextSize, 0);
        }

        mRequestedExpandedTitleTextSize =
                a.getDimensionPixelSize(R.styleable.CollapsingTitleLayout_expandedTextSize,
                        mCollapsedTitleTextSize);

        final int interpolatorId =
                a.getResourceId(R.styleable.CollapsingTitleLayout_textSizeInterpolator,
    ...
    public void setTextAppearance(int resId) {
        TypedArray atp =
                getContext().obtainStyledAttributes(resId, R.styleable.CollapsingTextAppearance);
        mTextPaint.setColor(
                atp.getColor(R.styleable.CollapsingTextAppearance_android_textColor, Color.WHITE));
        mCollapsedTitleTextSize =
                atp.getDimensionPixelSize(R.styleable.CollapsingTextAppearance_android_textSize, 0);
        atp.recycle();
    ...
    private void calculateOffsets() {
        ...
        final float textSizeOffset =
                mTextSizeInterpolator != null ? mTextSizeInterpolator.getInterpolation(
                        mScrollOffset) : offset;
        ...
        mTextRight =
                interpolate(getWidth() - mExpandedMarginRight, mToolbarContentBounds.right, offset);
    ...
    public void draw(Canvas canvas) {
        ...
        if (mTitleToDraw != null) {
            ...
            if (DEBUG_DRAW) {
                ...
                canvas.drawRect(mTextLeft, y - h + descent, mTextRight, y + descent,
                        DEBUG_DRAW_PAINT);
            }
    ...
    private void setInterpolatedTextSize(final float textSize) {
        ...
        if (isClose(textSize, mCollapsedTitleTextSize)
            ...
            final CharSequence title =
                    TextUtils.ellipsize(mTitle, mTextPaint, mTextRight - mTextLeft,
